...
const Activity = ({ type, prompt, context, pageId, activityId }) => {
        ...
        localFileStorage.get(storageKey).then(content => {
          if (content) {
            if (type === 'text') {
              ...
              setFileValue({ ...metadata, content });
            }
          } else if (metadata.content) {
            // Fallback for any existing Base64 data already in localStorage
            setFileValue(metadata);
          }
        });
      } catch (e) {
        // Fallback for old simple-string text data
  ...
  const handleTextChange = (e) => {
    ...
    // Auto-save logic: saves 1 second after user stops typing
    if (saveTimeoutRef.current) clearTimeout(saveTimeoutRef.current);
    saveTimeoutRef.current = setTimeout(() => {
      // Save index metadata to localStorage, content to IndexedDB
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      // Save metadata to localStorage, content to IndexedDB
      ...
      {type === 'text' ? (
        ...
        <div className="markdown-page__activity-file-zone">
          {!fileValue ? (
            ...
            <div className="markdown-page__activity-file-preview">
              {fileValue.type.startsWith('image/') ? (
                <img src={fileValue.content} alt="Uploaded preview" />
              ) : (
                <button 
                  className="markdown-page__activity-pdf-link"
                  onClick={() => {
                    const win = window.open();
                    win.document.write(`<iframe src="${fileValue.content}" frameborder="0" style="border:0; top:0px; left:0px; bottom:0px; right:0px; width:100%; height:100%;" allowfullscreen></iframe>`);
                  }}
                >
                  📄 View PDF: {fileValue.name}
                </button>
              )}
              <button className="markdown-page__activity-delete-btn" onClick={removeFile}>Delete File</button>
            </div>
          )}
        </div>
      )}
...
const MarkdownPage = ({ content = '', pageId = 'default-page', title = 'Content', markdownComponents = {}, metadata }) => {
  ...
  useEffect(() => {
    ...
    if (metadata.title) {
      document.title = `${metadata.title} | Tech for Us`;
    }
    ...
    const imageUrl = metadata.ogImage?.startsWith('http') 
      ? metadata.ogImage 
      : window.location.origin + (metadata.ogImage || '/og-image-default.jpg');
  ...
  useEffect(() => {
    const computePositions = () => {
      ...
      const items = annotations.map((a) => {
        ...
        // Try to find the ACTUAL highlight element in the content area
        const highlightEl = contentRef.current?.querySelector(`.markdown-page__annotated-text[data-annotation-id="${a.id}"]`);

        let targetY;
        if (highlightEl) {
          // If text still exists, use its actual current position (handles shifting content)
          const hRect = highlightEl.getBoundingClientRect();
          targetY = hRect.top - listRect.top + listScrollTop;
        } else {
          // Fallback logic for orphaned annotations using heading-relative positioning
          ...
          if (a.nearHeading && contentRef.current) {
            ...
            if (targetH) {
              const hRect = targetH.getBoundingClientRect();
              targetY = hRect.top + (a.headingOffset || 0) - listRect.top + listScrollTop;
              foundAnchor = true;
            }
          }

          if (!foundAnchor) {
            // Fallback to stored absolute position if text and heading are both gone
            targetY = contentRect.top + (a.textPosition || 0) - listRect.top + listScrollTop;
          }
        }
  ...
  // Helper to find the nearest heading above the current selection for anchoring
  const getNearestHeading = () => {
    ...
    headingElements.forEach(el => {
      ...
      // Find the heading that is above the selection and closest to it
  ...
  // Export data for the current page only
  const handleExportPageData = async () => {
    ...
    // 1. Pack page-specific data from localStorage
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key === `annotations-${pageId}` || key.startsWith(`activity-${pageId}-`)) {
        backup.localStorage[key] = localStorage.getItem(key);
      }
    }

    // 2. Pack matching file contents from IndexedDB
    ...
    link.href = url;
    link.download = `tech-for-us-${pageId}-${new Date().toISOString().split('T')[0]}.json`;
    link.click();
            ...
            {annotations.map(annotation => (
              ...
              >
                {editingAnnotationId === annotation.id ? (
                  ...
                  <>
                    {orphanedIds.has(annotation.id) && (
                      <div style={{ fontSize: '0.7rem', color: '#ff4444', fontWeight: 'bold', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <span>⚠️</span> CONTEXT MISSING (PAGE UPDATED)
                      </div>
                    )}
            ...
            annotations.filter(a => a.id === focusedAnnotationId).map(annotation => (
              <div key={annotation.id}>
                {editingAnnotationId === annotation.id ? (
                  ...
                  <>
                    {orphanedIds.has(annotation.id) && (
                      <div style={{ fontSize: '0.7rem', color: '#ffaaaa', fontWeight: 'bold', marginBottom: '6px' }}>
                        ⚠️ Context Missing (Page Updated)
                      </div>
                    )}
